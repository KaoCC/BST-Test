/* THIS TEMPORARY c SOURCE FILE WAS GENERATED BY snack version 0.9.0 */
/* THIS FILE : /home/liu/BinarySearchTree/kernel.snackwrap.c  */
/* INPUT FILE: /home/liu/BinarySearchTree/kernel.cl  */
/* UPDATED CL: /tmp/snk_7572/updated.cl  */
/*                               */ 
...
hsa_ext_module_t*                kernel_BrigModule;
hsa_agent_t                      kernel_Agent;
hsa_ext_program_t                kernel_HsaProgram;
hsa_executable_t                 kernel_Executable;
hsa_region_t                     kernel_KernargRegion;
int                              kernel_FC = 0; 

/* Global variables */
hsa_queue_t*                     Sync_CommandQ;
hsa_signal_t                     Sync_Signal; 
#include "kernel_brig.h" 

status_t kernel_InitContext(){
    ...
    err = hsa_iterate_agents(get_gpu_agent, &kernel_Agent);
    if(err == HSA_STATUS_INFO_BREAK) { err = HSA_STATUS_SUCCESS; }
    ErrorCheck(Getting a gpu agent, err);

    /* Query the name of the agent.  */
    char name[64] = { 0 };
    err = hsa_agent_get_info(kernel_Agent, HSA_AGENT_INFO_NAME, name);
    ErrorCheck(Querying the agent name, err);
    /* printf("The agent name is %s.\n", name); */

    /* Query the maximum size of the queue.  */
    uint32_t queue_size = 0;
    err = hsa_agent_get_info(kernel_Agent, HSA_AGENT_INFO_QUEUE_MAX_SIZE, &queue_size);
    ErrorCheck(Querying the agent maximum queue size, err);
    /* printf("The maximum queue size is %u.\n", (unsigned int) queue_size); */

    /* Load the BRIG binary.  */
    kernel_BrigModule = (hsa_ext_module_t*) &HSA_BrigMem;

    /* Create hsa program.  */
    memset(&kernel_HsaProgram,0,sizeof(hsa_ext_program_t));
    err = hsa_ext_program_create(HSA_MACHINE_MODEL_LARGE, HSA_PROFILE_FULL, HSA_DEFAULT_FLOAT_ROUNDING_MODE_DEFAULT, NULL, &kernel_HsaProgram);
    ErrorCheck(Create the program, err);

    /* Add the BRIG module to hsa program.  */
    err = hsa_ext_program_add_module(kernel_HsaProgram, kernel_BrigModule);
    ErrorCheck(Adding the brig module to the program, err);

    /* Determine the agents ISA.  */
    hsa_isa_t isa;
    err = hsa_agent_get_info(kernel_Agent, HSA_AGENT_INFO_ISA, &isa);
    ...
    err = hsa_ext_program_finalize(kernel_HsaProgram, isa, 0, control_directives, "", HSA_CODE_OBJECT_TYPE_PROGRAM, &code_object);
    ErrorCheck(Finalizing the program, err);

    /* Destroy the program, it is no longer needed.  */
    err=hsa_ext_program_destroy(kernel_HsaProgram);
    ErrorCheck(Destroying the program, err);

    /* Create the empty executable.  */
    err = hsa_executable_create(HSA_PROFILE_FULL, HSA_EXECUTABLE_STATE_UNFROZEN, "", &kernel_Executable);
    ErrorCheck(Create the executable, err);

    /* Load the code object.  */
    err = hsa_executable_load_code_object(kernel_Executable, kernel_Agent, code_object, "");
    ErrorCheck(Loading the code object, err);

    /* Freeze the executable; it can now be queried for symbols.  */
    err = hsa_executable_freeze(kernel_Executable, "");
    ErrorCheck(Freeze the executable, err);

    /* Find a memory region that supports kernel arguments.  */
    kernel_KernargRegion.handle=(uint64_t)-1;
    hsa_agent_iterate_regions(kernel_Agent, get_kernarg_memory_region, &kernel_KernargRegion);
    err = (kernel_KernargRegion.handle == (uint64_t)-1) ? HSA_STATUS_ERROR : HSA_STATUS_SUCCESS;
    ErrorCheck(Finding a kernarg memory region, err);

    /*  Create a queue using the maximum size.  */
    err = hsa_queue_create(kernel_Agent, queue_size, HSA_QUEUE_TYPE_SINGLE, NULL, NULL, UINT32_MAX, UINT32_MAX, &Sync_CommandQ);
    ...
    for ( stream_num = 0 ; stream_num < SNK_MAX_STREAMS ; stream_num++){
       ...
       err=hsa_queue_create(kernel_Agent, queue_size, HSA_QUEUE_TYPE_SINGLE, NULL, NULL, UINT32_MAX, UINT32_MAX, &Stream_CommandQ[stream_num]);
       ErrorCheck(Creating the Stream Command Q, err);
    }

    return STATUS_SUCCESS;
} /* end of _kernel_InitContext */
...
hsa_executable_symbol_t          Find_Symbol;
int                              Find_FK = 0 ; 
status_t                         Find_init();
uint64_t                         Find_Kernel_Object;
uint32_t                         Find_Kernarg_Segment_Size; /* May not need to be global */
uint32_t                         Find_Group_Segment_Size;
uint32_t                         Find_Private_Segment_Size;

/* ------  Start of SNACK function Find ------ */ 
extern void Find(void* node,void* out,int* key_list, const snk_lparm_t * lparm) {
   /* Kernel initialization has to be done before kernel arguments are set/inspected */ 
   if (Find_FK == 0 ) { 
     status_t status = Find_init(); 
   ...
   thisKernargAddress = malloc(Find_Kernarg_Segment_Size); 
   struct Find_args_struct {
      uint64_t arg0;
      uint64_t arg1;
      uint64_t arg2;
      uint64_t arg3;
   ...
   struct Find_args_struct* Find_args ; 
   /* Setup kernel args */ 
   Find_args = (struct Find_args_struct*) thisKernargAddress; 
   Find_args->arg0=0 ; 
   Find_args->arg1=0 ; 
   Find_args->arg2=0 ; 
   Find_args->arg3=0 ; 
   ...
   Find_args->arg6 = node ; 
   Find_args->arg7 = out ; 
   Find_args->arg8 = key_list ; 
   int needs_return_task = 0;
    ...
    this_aql->kernel_object = Find_Kernel_Object;
    this_aql->private_segment_size = Find_Private_Segment_Size;
    this_aql->group_segment_size = Find_Group_Segment_Size;
...
/* ------  End of SNACK function Find ------ */ 
extern status_t Find_init(){

    if (kernel_FC == 0 ) {
       status_t status = kernel_InitContext();
       if ( status  != STATUS_SUCCESS ) return; 
       kernel_FC = 1;
    }
    ...
    /* printf("Kernel name Find_: Looking for symbol %s\n","__OpenCL_Find_kernel"); */
    err = hsa_executable_get_symbol(kernel_Executable, "", "&__OpenCL_Find_kernel", kernel_Agent , 0, &Find_Symbol);
    ErrorCheck(Extract the symbol from the executable, err);

    /* Extract dispatch information from the symbol */
    err = hsa_executable_symbol_get_info(Find_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT, &Find_Kernel_Object);
    ErrorCheck(Extracting the symbol from the executable, err);
    err = hsa_executable_symbol_get_info(Find_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_KERNARG_SEGMENT_SIZE, &Find_Kernarg_Segment_Size);
    ErrorCheck(Extracting the kernarg segment size from the executable, err);
    err = hsa_executable_symbol_get_info(Find_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_GROUP_SEGMENT_SIZE, &Find_Group_Segment_Size);
    ErrorCheck(Extracting the group segment size from the executable, err);
    err = hsa_executable_symbol_get_info(Find_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_PRIVATE_SEGMENT_SIZE, &Find_Private_Segment_Size);
...
/* Kernel specific globals, one set for each kernel  */
hsa_executable_symbol_t          Insert_Symbol;
int                              Insert_FK = 0 ; 
status_t                         Insert_init();
uint64_t                         Insert_Kernel_Object;
uint32_t                         Insert_Kernarg_Segment_Size; /* May not need to be global */
uint32_t                         Insert_Group_Segment_Size;
uint32_t                         Insert_Private_Segment_Size;

/* ------  Start of SNACK function Insert ------ */ 
extern void Insert(void* rootNode,void* dataNode, const snk_lparm_t * lparm) {
   /* Kernel initialization has to be done before kernel arguments are set/inspected */ 
   if (Insert_FK == 0 ) { 
     status_t status = Insert_init(); 
     if ( status  != STATUS_SUCCESS ) return; 
     Insert_FK = 1; 
   } 
   /* Allocate the kernel argument buffer from the correct region. */ 
   ...
   /* HSA 1.0F has a bug that serializes all queue operations when hsa_memory_allocate is used. 
	  Revert back to hsa_memory_allocate once bug is fixed. */ 
   thisKernargAddress = malloc(Insert_Kernarg_Segment_Size); 
   struct Insert_args_struct {
      uint64_t arg0;
      uint64_t arg1;
      uint64_t arg2;
      uint64_t arg3;
   ...
   struct Insert_args_struct* Insert_args ; 
   /* Setup kernel args */ 
   Insert_args = (struct Insert_args_struct*) thisKernargAddress; 
   Insert_args->arg0=0 ; 
   Insert_args->arg1=0 ; 
   Insert_args->arg2=0 ; 
   Insert_args->arg3=0 ; 
   Insert_args->arg4=0 ; 
   Insert_args->arg5=0 ; 
   Insert_args->arg6 = rootNode ; 
   Insert_args->arg7 = dataNode ; 
   int needs_return_task = 0;
    ...
    /*  Bind kernel argument buffer to the aql packet.  */
    this_aql->kernarg_address = (void*) thisKernargAddress;
    this_aql->kernel_object = Insert_Kernel_Object;
    this_aql->private_segment_size = Insert_Private_Segment_Size;
    this_aql->group_segment_size = Insert_Group_Segment_Size;
...
/* ------  End of SNACK function Insert ------ */ 
extern status_t Insert_init(){
    ...
    /* Extract the symbol from the executable.  */
    /* printf("Kernel name Insert_: Looking for symbol %s\n","__OpenCL_Insert_kernel"); */
    err = hsa_executable_get_symbol(kernel_Executable, "", "&__OpenCL_Insert_kernel", kernel_Agent , 0, &Insert_Symbol);
    ErrorCheck(Extract the symbol from the executable, err);

    /* Extract dispatch information from the symbol */
    err = hsa_executable_symbol_get_info(Insert_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT, &Insert_Kernel_Object);
    ErrorCheck(Extracting the symbol from the executable, err);
    err = hsa_executable_symbol_get_info(Insert_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_KERNARG_SEGMENT_SIZE, &Insert_Kernarg_Segment_Size);
    ErrorCheck(Extracting the kernarg segment size from the executable, err);
    err = hsa_executable_symbol_get_info(Insert_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_GROUP_SEGMENT_SIZE, &Insert_Group_Segment_Size);
    ErrorCheck(Extracting the group segment size from the executable, err);
    err = hsa_executable_symbol_get_info(Insert_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_PRIVATE_SEGMENT_SIZE, &Insert_Private_Segment_Size);
    ErrorCheck(Extracting the private segment from the executable, err);
